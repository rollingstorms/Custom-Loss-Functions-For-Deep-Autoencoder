...
class AudioDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index=0, data=None, return_filename=False):
        ...
        if self.num_tiles == None:
            ...
            if self.num_tiles > 1: 
                slice_size = (original_width - self.image_width) // (self.num_tiles - 1)
            else:
                slice_size = 0
            ...
            for idx, img in enumerate(X):
                ...
                for i in range(self.num_tiles):
                    tiles.append(img[:,i*slice_size:(i*slice_size)+self.image_width,:])
                all_tiles.append(tiles)
                        
            X = tf.convert_to_tensor(np.array(all_tiles))
            y = X
    ...
    def take(self, index, return_filename=False):
        return self.__getitem__(index=index, return_filename=return_filename)

    def get_tensors_from_data(self, data):
        return self.__getitem__(data=data)
